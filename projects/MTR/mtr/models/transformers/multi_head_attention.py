...
class MultiheadAttention(nn.Module):
    ...
    def forward(
        ...
        Outputs:
            ...
            - **attn_output_weights** - Attention output weights of shape :math:`(N, L, S)`,
              where :math:`N` is the batch
...
def _in_projection_packed(
    ...
    r"""Performs the in-projection step of the attention operation, using packed weights.
...
def multi_head_attention_forward(
    ...
    if attn_mask is not None:
        ...
        if attn_mask.dim() == 2:
            ...
            if attn_mask.shape != correct_3d_size:
                raise RuntimeError(f"{attn_mask.shape=} != {correct_3d_size=}.")
        else:
    ...
    # NOTE: https://github.com/pytorch/pytorch/issues/99701
    if static_k is None:
    ...
    if key_padding_mask is not None:
        ...
        if attn_mask is None:
            ...
            attn_mask = attn_mask.masked_fill(key_padding_mask, -1000.0)  # NOTE: float("-inf") make nan TRT output

    # convert mask to float
    if attn_mask is not None and attn_mask.dtype == torch.bool:
        new_attn_mask = torch.zeros_like(attn_mask, dtype=torch.float)
        new_attn_mask.masked_fill_(attn_mask, -1000.0)  # NOTE: float("-inf") make nan TRT output
