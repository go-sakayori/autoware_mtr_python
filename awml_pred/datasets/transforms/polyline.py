...
class TargetCentricPolyline:
    ...
    def _do_transform(
        ...
        """
        num_target, _ = current_target.shape

        polylines[..., :3] -= current_target.xyz[:, None, None]
    ...
    def __call__(self, info: dict) -> dict:
        ...
        num_target = scenario.num_agent if predict_all_agents else scenario.num_target

        static_map: AWMLStaticMap = info["static_map"]
        ...
        ret_polylines, ret_polylines_mask = self._do_transform(ret_polylines, ret_polylines_mask, current_target)
